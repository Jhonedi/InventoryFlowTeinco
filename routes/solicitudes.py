...
def nueva_solicitud():
    ...
    if request.method == 'POST':
        try:
            ...
            for i, repuesto_id in enumerate(repuesto_ids):
                ...
                # Obtener precio actual del repuesto
                repuesto = execute_query(
                    "SELECT precio_venta, cantidad_actual, cantidad_reservada FROM repuestos WHERE id = %s",
    ...
    # Obtener repuestos con stock disponible (ordenados alfabéticamente)
    repuestos = execute_query("""
        SELECT r.id, r.codigo, r.nombre, r.precio_venta,
...
def ver_solicitud(id):
    ...
    solicitud = execute_query("""
        ...
        FROM solicitudes_repuestos s
        JOIN usuarios ut ON s.tecnico_id = ut.id
        LEFT JOIN usuarios ua ON s.aprobado_por = ua.id
        LEFT JOIN usuarios ue ON s.entregado_por = ue.id
        LEFT JOIN usuarios uf ON s.facturado_por = uf.id
        JOIN clientes c ON s.cliente_id = c.id
        JOIN vehiculos_clientes v ON s.vehiculo_id = v.id
...
@solicitudes_bp.route('/api/vehiculos-cliente/<int:cliente_id>')
@login_required
def api_vehiculos_cliente(cliente_id):
    """Obtener vehículos de un cliente"""
    vehiculos = execute_query("""
        SELECT vc.id, vc.placa, vc.color, vc.anio,
