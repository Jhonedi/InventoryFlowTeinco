...
def lista_facturas():
    ...
    total = execute_query(f"""
        ...
        JOIN vehiculos_clientes v ON f.vehiculo_cliente_id = v.id
        WHERE {where_sql}
    ...
    facturas = execute_query(f"""
        ...
        JOIN clientes c ON f.cliente_id = c.id
        JOIN vehiculos_clientes v ON f.vehiculo_cliente_id = v.id
        JOIN usuarios uv ON f.vendedor_id = uv.id
        WHERE {where_sql}
...
def ver_factura(id):
    ...
    factura = execute_query("""
        ...
        JOIN marcas_vehiculos ma ON mv.marca_id = ma.id
        JOIN usuarios uv ON f.vendedor_id = uv.id
        LEFT JOIN usuarios ua ON f.anulado_por = ua.id
    ...
    detalles = execute_query("""
        ...
        JOIN repuestos r ON df.repuesto_id = r.id
    ...
    pagos = execute_query("""
        ...
        LEFT JOIN usuarios u ON pf.recibido_por = u.id
...
def _procesar_factura_pagada(factura_id, factura, user):
    ...
    for detalle in detalles:
        ...
        movimiento_id = execute_query("""
            ...
            VALUES (
                %s,
                (SELECT id FROM tipos_movimiento WHERE nombre = 'Venta/Facturación' LIMIT 1),
                %s, %s, %s, %s, 'FACTURADO',
                %s
            )
        """, (
...
def _verificar_alertas_stock(repuesto_id):
    """Verifica y crea alertas de stock bajo para un repuesto"""
    try:
